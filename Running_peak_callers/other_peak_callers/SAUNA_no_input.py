...
class Configuration:
    def __init__(self, filename: str, minVal: int, length: int,locusBegin: int, locusLength: int, nucLength: int, chromosome: str):
        ...
        self.intervals = [Interval(0, locusLength, Interval.TYPE.DNA)]
        self.numOfNucleosomes = 0
...
class Energy:
    def __init__(self, parent_dir: str, filename: str, probabilities: np.ndarray, locusBegin: int, locusEnd: int, bindingEnergy: float):
        ...
        # Handle probabilities outside the valid range
...
class EnergyFactory:
    def __init__(self, parent_dir,filename,pReads, locusBegin, locusEnd):
        ...
        del length
        self.nucCenters[0:len(pReads)] = pReads
        del pReads
        gc.collect()
    ...
    def give_energy(self, sigma, binding_energy):
        ...
        self.nucCenters = self.nucCenters - min_value
...
#         gc.collect()
        ...
        # add flanking DNA
        self.locusBegin = max(0, self.minValue - GENERIC_NUC_LENGTH)
        self.locusEnd = self.maxValue + GENERIC_NUC_LENGTH
        ...
        for interval in config.intervals:
            if interval.getType() == Interval.TYPE.NUC:
                out.write(f"{config.getChromosome()}\t"
                          f"{config.getLocusBegin() + interval.getBegin()}\t"
                          f"{config.getLocusBegin() + interval.getEnd()}\n")
        out.flush()
        del config
        gc.collect()
...
def main():
    try:
            ...
            for line in paramFile:
                ...
                if key == "SimSteps":
                    if value == 'default':
                        ...
                        simSteps = int(value)
                    del length
                elif key == "StepsToSave":
                    stepsToSave = int(value)
                elif key == "NucLength":
                    nucLength = int(value)
        ...
        print("have min and max values")
    
        config = Configuration(name,minVal, maxVal - minVal,locusBegin,locusEnd-locusBegin, nucLength, chrom)
        ...
        bedFilename = outputFilebase + ".no_input_result.bed"
